...
export default function AdminChangePasswordPage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await fetch('/api/admin/password', {
        ...
        body: JSON.stringify({ current: data.current, next: data.new })
      });
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader title="تغيير كلمة المرور" description="قم بتحديث كلمة المرور الخاصة بحساب الأدمن" />
      <div className="bg-white rounded-xl shadow p-6 max-w-xl">
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
          <div className="space-y-2">
            <Label htmlFor="current">كلمة المرور الحالية</Label>
            <Input id="current" type="password" aria-invalid={errors.current ? true : undefined} {...register("current", { required: "مطلوب" })} />
          ...
          <div className="space-y-2">
            ...
            <Input id="new" type="password" aria-invalid={errors.new ? true : undefined} {...register("new", { required: "مطلوب" })} />
