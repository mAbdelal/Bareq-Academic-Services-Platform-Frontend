  ...
  return (
    <div className="h-[90vh] flex flex-col md:flex-row items-center justify-between mt-5 gap-10">
      ...
      <div className="flex-1 flex flex-col items-center lg:items-start justify-center lg:text-right">
        ...
        {/* Search Input */}
        <div className="flex items-center relative bg-background rounded-full shadow-md overflow-hidden w-full max-w-md transition-transform transform hover:scale-[1.02]">
          <Input
            type="text"
        ...
        <div className="flex flex-wrap justify-end gap-3 mt-4">
          {["الحاسوب", "الرياضيات", "الفيزياء", "الاقتصاد"].map((tag) => (
            <span
              key={tag}
              className="bg-primary/20 text-primary px-3 py-1 rounded-full cursor-pointer hover:bg-primary/40 transition-colors text-sm"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {categories.map((cat) => (
            <div key={cat.id} className="group text-right">
              ...
              {/* Hover Buttons Below Card */}
              <div className="mt-3 flex justify-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                <Button size="sm" variant="outline" className="bg-cardBg hover:bg-primary hover:text-white">
                  أكاديميين
                </Button>
                <Button size="sm" variant="outline" className="bg-cardBg hover:bg-primary hover:text-white">
                  أعمال
